let strafe = 0;
let secondaries = true;
let map = false;
//Set strafe to 1 if you want strafe and 0 if not
//Set secondaries to false if you want secondaries and vice versa
//Set map to true if you want asteroids and vice versa
let alien_array = [
  {code:19,level:0}
];
...
game.modding.commands.authorize = function(req){
...
game.modding.commands.unauthorize = function(req){
  let args=req.replace(/^\s+/,"").replace(/\s+/," ").split(" "),id=Number(args[1]||"NaN");
...
this.options = {
  ...
  map_name: 'Team dueling',
  friendly_colors: 2,
  weapons_store: secondaries,
  max_level: 8,
  soundtrack: "argon.mp3"
};
if (!map) this.options.custom_map = []; 
...
function setteam(ship){
  ...
  for (let ship of game.ships){
    let a = ship.custom.team;
    teams[a]++;
  }
  t = teams.indexOf(Math.min(...teams));
  ship.custom.team = t;
  configship(ship,t);
}

function configship(ship,t){
  ship.set({hue:hues[t],team:t,invulnerable:300,stats:88888888});
}
...
this.event = function(event, game){
  ...
  var ids = ["Next ship","Previous ship","Reset","Spectate","A-Speedster","Switch team"];
  var shc = ["1","2","3","4","5","6","7"];
  var w = [["Rockets",10],["Missiles",11],["Torpedos",12],["Mines",20],["Heavy Mines",21]];
  if (secondaries) ids.push("Torp");
  switch (event.name){
    case "ui_component_clicked":
      ...
      else if (component == "Switch team"){
        let t = Math.abs(ship.custom.team-1);
        ship.set({hue:hues[t],team:t})
        ship.custom.team = t;
      }      
      else if (component == "Weapons"){
        for (let i=0; i<w.length; i++){
          ship.setUIComponent({
            id: `"${w[i][0]}"`,
            position: [19+i*12,31,12,10],
            clickable: true,
            visible: true,
            shortcut: sch[i],
            components: [
              {type:"box",position:[0,0,100,100],fill:"rgba(68, 85, 102, 0)",stroke:"#CDE",width:4},
              {type:"text",position:[5,35,90,40],value:`${w[i][0]} [${sch[i]}]`,color:"#CDE"},
            ]
          });
        }
      }            
